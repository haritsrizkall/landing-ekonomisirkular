...
const ScoresIndividu = () => {
    const [detailModal, setDetailModal] = useState(false);
    const [loadingDownload, setLoadingDownload] = useState(false);
    const [selectedData, setSelectedData] = useState();
    const [deleteModal, setDeleteModal] = useState(false);
    const { isError, isLoading, data: scores, status, error } = useQuery("scoresIndividu", scoreAPI.getIndividuScores);
    const queryClient = useQueryClient();
    const mutationDelete = useMutation(scoreAPI.delete, {
        onSuccess: () => {
            queryClient.invalidateQueries("scoresIndividu")
        }
    })
    const data = useMemo(
        () => scores,
    ...
    const handleDownload = (e) => {
        ...
        scoreAPI.downloadScoresIndividu().then((res) => {
            const url = window.URL.createObjectURL(new Blob([res.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'scores_individu.xlsx');
    ...
    const handleDownloadRecap = (e) => {
        e.preventDefault();
        setLoadingDownload(true)
        answersAPI.downloadRecapAnswer("individu").then((res) => {
            const url = window.URL.createObjectURL(new Blob([res.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `recap_answer_individu.xlsx`);
            document.body.appendChild(link);
            link.click();
            setLoadingDownload(false)
        });
    ...
    return (
        <> 
            ...
            <ScoreDetailModal isVisible={detailModal} setIsVisible={setDetailModal} score={selectedData}/>
