...
const QuestionGroupAdd = ({isVisible, setIsVisible}) => {
    const queryClient = useQueryClient();
    const [questionSetId, setQuestionSetId] = useState(1);
    const [name, setName] = useState("");
    const [description, setDescription] = useState("");

    const { isError, isLoading, data: questionSets, status, refetch } = useQuery("questionSets", questionSetsAPI.getAll);
    const mutation = useMutation(questionGroupAPI.addQuestionGroup, {
        onSuccess: () => {
            console.log("Success");
    ...
    return (
        <div className={isVisible ? "flex flex-col justify-center items-center w-screen h-screen z-30 fixed top-0 right-0": "hidden"}>
            ...
            <div className="bg-white py-5 w-100 rounded flex flex-col justify-center text-center drop-shadow-lg px-2">
                <div>
                    <h1 className="text-xl font-semibold mb-5   ">Add Question Group</h1>
                </div>
                {
                    mutation.isError && (
                        <div onCLick={() =>{
                ...
                <div className="flex w-full my-1">
                    ...
                    <div className="basis-4/5 mr-5">
                        <textarea rows={4} type="text" className="py-2 pl-3 bg-slate-100 outline-none w-full" value={name} onChange={(e) => {
                ...
                <div className="flex w-full my-1">
                    ...
                    <div className="basis-4/5 mr-5">
                        <textarea rows={4} type="text" className="py-2 pl-3 bg-slate-100 outline-none w-full" value={description} onChange={(e) => {
                ...
                <div className="flex w-full my-1">
                    ...
                    <div className="basis-4/5 mr-5">
                        <select className="py-2 pl-3 bg-slate-100 outline-none w-full" onChange={(e) => setQuestionSetId(parseInt(e.target.value))} value={questionSetId}>
...
export default QuestionGroupAdd