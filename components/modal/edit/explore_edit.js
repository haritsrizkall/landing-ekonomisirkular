...
const ExploreEdit = ({isVisible, setIsVisible, explore}) => {
    ...
    useEffect(() => {
        setTitle(explore.title)
        setContent(explore.content)
    }, [explore])
    ...
    return (
        <div className={isVisible ? "flex flex-col justify-center items-center w-screen h-screen z-30 fixed top-0 right-0": "hidden"}>
            ...
            <div className="bg-white py-5 w-100 rounded flex flex-col justify-center text-center drop-shadow-lg px-2">
                <div>
                    <h1 className="text-xl font-semibold mb-5   ">Edit Explore</h1>
                </div>
                {
                    mutation.isError && (
                        <div onCLick={() =>{
                ...
                </div><div className="flex w-full my-1">
                    <div className="mr-0 basis-1/5 text-left">
                        <p className="font-medium">Content</p>
                    </div>
                    <div className="basis-4/5 mr-5">
                        <textarea rows={10} type="text" className="py-2 pl-3 bg-slate-100 outline-none w-full" value={content} onChange={(e) => {
